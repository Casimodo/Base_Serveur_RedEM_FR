...
AddEventHandler(
    ...
    function(_type, data, target, LockerID, stashId)        local _source = source
        local _source = source
    ...
    function(data, toPlayer)
        ...
        if removeItem(data.name, data.amount, data.meta, identifier, charid) then
            ...
            if addItem(data.name, data.amount, data.meta, identifier2, charid2) then
                ...
                TriggerClientEvent("redem_roleplay:NotifyRight", toPlayer, "On vous a donné "..data.amount.."x "..itemData.label, 3000)
                TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Vous avez donné "..data.amount.."x "..itemData.label, 3000)
            else
...
AddEventHandler(
    ...
    function(id)
        ...
        if
            ...
            TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Vous ne pouvez pas prendre ça!", 3000)
        end
...
AddEventHandler(
    ...
    function(data)
        ...
        if itemData.type == "item_weapon" then
            ...
            TriggerClientEvent("redem_roleplay:NotifyLeft", _source, "Arme équipé", data.label, "menu_textures", "menu_icon_holster", 1000)
            TriggerClientEvent("redemrp_inventory:PlaySound", _source, 1)
        end
    end
)
...
RegisterCommand("listitems", function(source,args)
    ...
    if Player.GetGroup() == "superadmin" or Player.GetGroup() == "admin" then
        ...
        return TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Pas les permissions suffisante!", 3000)
    end
end)
    ...
    function(source, args)
        ...
        if Player.GetGroup() == "admin" or Player.GetGroup() == "superadmin" then
            ...
            if not TargetPlayer then
                return TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Joueur invalide.", 3000)
            end
            ...
            if retVal ~= -1 then
                if retVal then
                    TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Vous avez donné "..GetPlayerName(tonumber(args[1])).." "..args[3].."x "..args[2].."!", 3000)
                    TriggerClientEvent("redem_roleplay:NotifyRight", tonumber(args[1]), "On vous a donné "..args[3].."x "..args[2].." par le staff!", 3000)
                    TriggerClientEvent(
                        "redemrp_inventory:SendItems",
                        tonumber(args[1]),
                        PrepareToOutput(Inventory[identifier .. "_" .. charid]),
                    ...
                    TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Il y a eu une erreur. Le joueur n'a plus de place?", 3000)
                end
            else
                TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Il y a eu une erreur. L'item n'existe pas?", 3000)
            end
        else
            TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Pas les permissions suffisante!", 3000)
        end
    end
)
...
RegisterCommand(
    ...
    function(source, args)
        ...
        if Player.GetGroup() == "admin" or Player.GetGroup() == "superadmin" then
            ...
            if retVal ~= -1 then
                TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Vous avez retiré "..args[3].."x "..args[2].." de "..GetPlayerName(tonumber(args[1])).."!", 3000)
                TriggerClientEvent("redem_roleplay:NotifyRight", tonumber(args[1]), "Vous aviez "..args[3].."x "..args[2].." qui a été retiré par le staff!", 3000)
                TriggerClientEvent(
                    "redemrp_inventory:SendItems",
                    tonumber(args[1]),
                    PrepareToOutput(Inventory[identifier .. "_" .. charid]),
...
function addItem(name, amount, meta, identifier, charid, lvl)
    ...
    if _name == nil or _amount == nil then
        print("^6[redemrp_inventory]^0 Paramètres d'item invalide (name or amount)")
        return -1
    end
    if _amount >= 0 then
        if not Config.Items[_name] then
            print("^6[redemrp_inventory]^0 Tentative d'ajout d'item invalide. (".._name..")")
            return -1
        end
...
function removeItem(name, amount, meta, identifier, charid)
    ...
    if _amount >= 0 then
        ...
        if item then
            ...
            print("Aucune possibilité de retirer l'item? ["..name..", "..amount..", "..json.encode(meta).."]")
        end
        local charName = ""
        local FoundPlayer = false
    end
...
function addItemStash(source, name, amount, meta, stashId)
    ...
    if _amount >= 0 then
        ...
        if itemData.type == "item_weapon" or itemData.type == "item_letter" then
            ...
            TriggerClientEvent("redemrp_inventory:client:WeightNotif", _source, "Poid: ~n~"..string.format("%.2f", weight + (itemData.weight)).."kg / "..string.format("%.2f", weightLimit).."kg", 2000)
            ...
            --("Boss stash weight: ".. weight .." vs ".. weightLimit)
            TriggerClientEvent("redemrp_inventory:client:WeightNotif", _source, "Poid: ~n~"..string.format("%.2f", weight + (itemData.weight * amount)).."kg / "..string.format("%.2f", weightLimit).."kg", 2000)
            --(weight + (itemData.weight * amount))
...
function removeItemStash(source, name, amount, meta, stashId)
    ...
    TriggerClientEvent("redemrp_inventory:client:WeightNotif", _source, "Poid: ~n~"..string.format("%.2f", GetStashWeight(stashId)).."kg / "..string.format("%.2f", weightLimit).."kg", 2000)
    return output
end
...
RegisterServerEvent("redemrp_inventory:GetPlayer",
    function(target, hogtied)
        ...
        TriggerEvent("redemrp_respawn:IsPlayerDead", _target, function(isDead)
            if isDead then
                return TriggerClientEvent("redem_roleplay:NotifyLeft", _source, "Vous ne pouvez pas chercher!", "On ne fouille pas un joueur mort.", "menu_textures", "menu_icon_alert", 3000)
            end
            if HandsUp[_target] or hogtied then
                if not exports["redem_roleplay"]:RedEM().CrimeDisabled then
                    local identifier_target = TargetPlayer.GetIdentifier()
                    local charid_target = TargetPlayer.GetActiveCharacter()
                    TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Dollar: $"..comma_value(string.format("%.2f", TargetPlayer.getMoney())), 3000)
                    TriggerClientEvent(
                        "redemrp_inventory:SendItems",
                        _source,
                        PrepareToOutput(Inventory[identifier .. "_" .. charid]),
                    ...
                    TriggerClientEvent("redem_roleplay:NotifyRight", _source, "Vous ne pouvez pas dépouiller quelqu'un <span style=\"color:lightblue\">30 minutes</span>avant le reboot!", 3000)
                end
            else
                TriggerClientEvent("redem_roleplay:NotifyLeft", _source, "Vous ne pouvez pas chercher!", "Le joueurs doit avoir les mains levé ou être ligoté.", "menu_textures", "menu_icon_alert", 3000)
            end
        end)
    end
)
...
RegisterServerEvent("redemrp_inventory:GetPlayerAsPolice",
    function(target)
        ...
        if Player.GetJob() == "police" or Player.GetJob() == "police2" or Player.GetJob() == "police3" or Player.GetJob() == "police4" or Player.GetJob() == "police5" or Player.GetJob() == "marshal" or Player.GetJob() == "ranger" then
            ...
            TriggerClientEvent("redem_roleplay:NotifyLeft", _source, "Vous ne pouvez pas chercher!", "Vous devez faire parti des shérifs.", "menu_textures", "menu_icon_alert", 3000)
        end
    end
)
...
RegisterServerEvent("redemrp_inventory:craft", function(data, id)
    ...
    if outputItem then
        ...
        if CanContinue then
            ...
            for v,k in pairs(data) do
                ...
                if  inputItem ~= "empty" then
                    ...
                    if checkitemdata.ItemAmount >= bpInputAmount*multipliedOutputItemAmount then
                        ...
                        return RedEM.Functions.NotifyLeft(_source, "Craft échoué!", "Il vous manque des items!", "menu_textures", "menu_icon_alert", 3000)
                    end
                end
            end
            ...
            RedEM.Functions.NotifyLeft(_source, "Craft échoué!", "Pas le bon job pour cette recette", "menu_textures", "menu_icon_alert", 3000)
        end
    end
end)
...
RegisterServerEvent("redemrp_inventory:server:FinishCraftingProgress", function(itemstoremove, outputItem, outputItemAmount)
    ...
    if itemData2.AddItem(outputItemAmount) then
        RedEM.Functions.NotifyLeft(_source, "Items Craft", "Vous avez crafté "..outputItemAmount.."x "..name.."!", "menu_textures", "menu_icon_alert", 3000)
    else
        RedEM.Functions.NotifyLeft(_source, "Craft échoué!", "Pas assez de place!", "menu_textures", "menu_icon_alert", 3000)
    end
end)
